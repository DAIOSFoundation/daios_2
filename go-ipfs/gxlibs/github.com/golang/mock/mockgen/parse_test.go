...
func TestFileParser_ParsePackage(t *testing.T) {
	...
	err = p.parsePackage("github.com/dai/go-ipfs/gxlibs/github.com/golang/mock/mockgen/tests/custom_package_name/greeter")
	if err != nil {
		t.Fatalf("Unexpected error: %v", err)
	}
...
func checkGreeterImports(t *testing.T, imports map[string]string) {
	...
	if validatorPackage, ok := imports["validator"]; !ok {
		...
		expectedValidatorPackage := "github.com/dai/go-ipfs/gxlibs/github.com/golang/mock/mockgen/tests/custom_package_name/validator"
	...
	if clientPackage, ok := imports["client"]; !ok {
		...
		expectedClientPackage := "github.com/dai/go-ipfs/gxlibs/github.com/golang/mock/mockgen/tests/custom_package_name/client/v1"
